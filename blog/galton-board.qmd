...
that given sufficiently large sample sizes, a collection of sample means
from the binomial distribution would approximate the normal distribution. 
The Galton board is not only very satisfying to watch, it also demonstrates one
of the most important concepts for statistical inference.

![The Galton board in action](https://upload.wikimedia.org/wikipedia/commons/transcoded/d/dc/Galton_box.webm/Galton_box.webm.720p.vp9.webm){width=70%}

Assume that you flip a coin and either get a tail or a head and want to 
investigate the probability of getting tail. If you were
to only flip the coin once, you would only get a value of either 0% or 100%.
However, as you increase your number of throws you will eventually approach 50% which is the true value.


If you then draw a many samples of a sufficiently large sample size, the sample
means will follow a normal distribution with the true mean as the mean of the distribution.
The larger the sample size is, the lower variance the distribution will have, see the figure below.
...
draw_samples(1000, 100) |> plot_samples()
draw_samples(1000, 1000) |> plot_samples()
```
...
probability that we obtain a sample mean of 0.2, if we belive the true mean
...
a hypothesis, we then collect data to test the hypothesis, and finally test
if we are able, or unable, to reject the hypothesis.
